...
trait MockListCheckpointsValidator extends TestSuite with MockFactory {
  ...
  object MockListCheckpointsValidator {

    def validate(data: ListCheckpointsRawData): CallHandler[List[MtdError]] =
      (mockListCheckpointsValidator.validate(_: ListCheckpointsRawData)).expects(data)
