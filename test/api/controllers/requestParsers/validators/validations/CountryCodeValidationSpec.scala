...
class CountryCodeValidationSpec extends UnitSpec {
  ...
  private val error = CountryCodeFormatError
    ...
    "validate" must {
      Seq(
        ...
        s"return an empty list for valid country code $code" in {
          CountryCodeValidation.validate(code, error) shouldBe NoValidationErrors
        }
      }

      "return a CountryCodeFormatError for an invalid country code" in {
        CountryCodeValidation.validate("notACountryCode", error) shouldBe List(error)
      }

      "return a CountryCodeFormatError for an invalid format country code" in {
        CountryCodeValidation.validate("FRANCE", error) shouldBe List(error)
      }

      "return a CountryCodeFormatError for an invalid rule country code" in {
        CountryCodeValidation.validate("XX", error) shouldBe List(error)
      }
    }
  }
