...
@Singleton
class CreateCheckpointService @Inject() (connector: CreateCheckpointConnector) extends BaseService {

  def createCheckpoint(request: CreateCheckpointRequest)(implicit
      ec: ExecutionContext,
      rc: RequestContext): Future[Either[ErrorWrapper, ResponseWrapper[CreateCheckpointResponse]]] =
    connector.createCheckpoint(request).map(_.leftMap(mapDownstreamErrors(stubErrorMap)))
