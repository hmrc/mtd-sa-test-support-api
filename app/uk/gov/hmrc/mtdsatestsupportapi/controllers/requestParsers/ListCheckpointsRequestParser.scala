...
class ListCheckpointsRequestParser @Inject() (val validator: ListCheckpointsValidator)
  ...
  override protected def requestFor(data: ListCheckpointsRawData): ListCheckpointsRequest = {
    data.nino match {
      case Some(nino) => ListCheckpointsRequest(data.vendorClientId, Some(Nino(nino)))
      case None       => ListCheckpointsRequest(data.vendorClientId, None)
    }
  }
