...
@Singleton
class BusinessDetailsConnector @Inject() (val http: HttpClientV2, val appConfig: AppConfig)
  ...
  def getMtdId(nino: String)(implicit hc: HeaderCarrier, ec: ExecutionContext, correlationId: String): Future[DownstreamOutcome[MtdIdReference]] = {
    ...
    implicit val context: ConnectorContext = ConnectorContext(config.hipDownstreamConfig)

    getWithBasicAuth(
      DownstreamUri[MtdIdReference]("etmp/RESTAdapter/itsa/taxpayer/business-details?nino=$nino"),
      DownstreamStrategy.basicAuthStrategy(config.hipDownstreamConfig)
    )
  }
